...
#                           self.track_best_vals[self.monitor][1])
        if self.save_model:
            ...
            if self.learn.new_best_values[self.monitor]:
                diff = np.abs(self.learn.best_values[self.monitor][1] - self.learn.best_values[self.monitor][2])
                if diff > self.min_delta:    
                    # ? is this correct? or comp between best new value and old value        
                    print(f"best checkpoint: {self.monitor}: {self.learn.best_values[self.monitor][2]}")
